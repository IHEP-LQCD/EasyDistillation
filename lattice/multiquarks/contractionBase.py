...
gammaDict = {
    ...
    "rho2": [Gamma4 @ Gamma1, Gamma4 @ Gamma2, Gamma4 @ Gamma3],
    "a1": [Gamma1 @ Gamma5, Gamma2 @ Gamma5, Gamma3 @ Gamma5],
    "b1": [Gamma2 @ Gamma3, Gamma3 @ Gamma1, Gamma1 @ Gamma2],
    "identity": (Gamma0),
    "gamma1": (Gamma1),
    "gamma2": (Gamma2),
    "gamma3": (Gamma3),
...
class Elementals(LatticeDesc):
    ...
    def load(self, isLoadAll=True, isHalfMom=False):
        ...
        self.data = cupy.zeros(
            shape=(self.calcMomNum, self.termNum, 2, self.TimeSlice, self.Nvec, self.Nvec), dtype=self.dtype
        )
        # !!! Note !!!
        ...
        timer0 = mytimer(f"loaded elementals({self.elementalFile})", self.TimeSlice * self.Nvec * self.Nvec)
        if isLoadAll:
            ...
            for iterm in range(self.termNum):
                print(
                    f"Use Deriv : {derivDict[self.derivType[iterm]]}, dataMomNum: {self.dataMomNum}, dataDnum: {self.dataDnum}"
                )
                ...
                for k in range(self.calcMomNum):
                    ...
                    self.data[k, iterm,
                              0] = cupy.asarray(elemCache[derivDict[self.derivType[iterm]], momPosList[k], :, :, :])
                    self.data[k, iterm, 1] = cupy.asarray(
                        elemCache[derivDict[self.derivType[iterm]], momPosList[-k - 1], :, :, :]
                    )
            ...
            for k in range(self.calcMomNum):
                for iterm in range(self.termNum):
                    # A = 0,  B = 1
                    print(
                        f"DBUG: derivDict[self.derivType[iterm]] = {derivDict[self.derivType[iterm]]}, k = {k}, momPosList[k] = {momPosList[k]}"
                    )
                    if self.elementalFile.endswith(".npy"):
                        # In jiangxy's style, shape = [Ntime, deriv, mom, Nvec, Nvec]
                        # 20220401: In xyjiang's test, memmap works better.
                        elemCache = np.memmap(
                            self.elementalFile, dtype=self.dtype, mode="r", shape=self.elemDataShape
                        )[derivDict[self.derivType[iterm]], momPosList[k], :, :, :]
                        self.data[k, iterm, 0] = cupy.asarray(elemCache)
                        elemCache = np.memmap(
                            self.elementalFile, dtype=self.dtype, mode="r", shape=self.elemDataShape
                        )[derivDict[self.derivType[iterm]], momPosList[-k - 1], :, :, :]
                        ...
                        offset = (
                            derivDict[self.derivType[iterm]] * self.dataMomNum + momPosList[k]
                        ) * count * self.dtype_size
                        self.data[k, iterm,
                                  0] = cupy.fromfile(self.elementalFile, dtype=self.dtype, offset=offset,
                                                     count=count).reshape(self.TimeSlice, self.Nvec, self.Nvec)
                        offset = (
                            derivDict[self.derivType[iterm]] * self.dataMomNum + momPosList[-k - 1]
                        ) * count * self.dtype_size
                        self.data[k, iterm,
                                  1] = cupy.fromfile(self.elementalFile, dtype=self.dtype, offset=offset,
                                                     count=count).reshape(self.TimeSlice, self.Nvec, self.Nvec)
...
class ElementalsAB(LatticeDesc):
    ...
    def load(self, isLoadAll=True, isHalfMom=False):
        ...
        self.dataA = cupy.zeros(
            shape=(self.calcMomNum, self.termNumA, self.TimeSlice, self.Nvec, self.Nvec), dtype=self.dtype
        )
        self.dataB = cupy.zeros(
            shape=(self.calcMomNum, self.termNumB, self.TimeSlice, self.Nvec, self.Nvec), dtype=self.dtype
        )
        # !!! Note !!!
        # self.momPairNum : momentum pair number, which multi 2 equal to |p| number when mom not 0
        # self.calcMomNum = len(momPosList)
        ...
        if isLoadAll:
            ...
            for k in range(self.calcMomNum):
                for itermA in range(self.termNumA):
                    print(
                        f"DBUG: derivDict[self.derivTypeA[iterm]] = {derivDict[self.derivTypeA[itermA]]}, k = {k}, momPosList[k] = {momPosList[k]}"
                    )
                    self.dataA[k, itermA] = cupy.asarray(
                        elemCache[derivDict[self.derivTypeA[itermA]], momPosList[k], :, :, :]
                    )
                for itermB in range(self.termNumB):
                    print(
                        f"DBUG: derivDict[self.derivTypeB[iterm]] = {derivDict[self.derivTypeB[itermB]]}, k = {k}, momPosList[k] = {momPosList[-k-1]}"
                    )
                    self.dataB[k, itermB] = cupy.asarray(
                        elemCache[derivDict[self.derivTypeB[itermB]], momPosList[-k - 1], :, :, :]
                    )
            ...
            for k in range(self.calcMomNum):
                for iterm in range(self.termNumA):
                    print(
                        f"DBUG: derivDict[self.derivTypeA[iterm]] = {derivDict[self.derivTypeA[iterm]]}, k = {k}, momPosList[k] = {momPosList[k]}"
                    )
                    if self.elementalFile.endswith(".npy"):
                        ...
                        elemCache = np.memmap(
                            self.elementalFile, dtype=self.dtype, mode="r", shape=self.elemDataShape
                        )[derivDict[self.derivTypeA[iterm]], momPosList[k], :, :, :]
                        ...
                        count = self.TimeSlice * self.Nvec * self.Nvec
                        offset = (
                            derivDict[self.derivTypeA[iterm]] * self.dataMomNum + momPosList[k]
                        ) * count * self.dtype_size
                        self.dataA[k, iterm] = cupy.fromfile(
                            self.elementalFile, dtype=self.dtype, offset=offset, count=count
                        ).reshape(self.TimeSlice, self.Nvec, self.Nvec)
                        print(f"Note: A: load binary file, elem shape:{self.data[k, iterm].shape}.")
                for iterm in range(self.termNumB):
                    print(
                        f"DBUG: derivDict[self.derivTypeB[iterm]] = {derivDict[self.derivTypeA[iterm]]}, k = {k}, momPosList[k] = {momPosList[k]}"
                    )
                    if self.elementalFile.endswith(".npy"):
                        # If In jiangxy's style, shape = [Ntime, deriv, mom, Nvec, Nvec]
                        # 20220401: In xyjiang's test, memmap works better.
                        ...
                        count = self.TimeSlice * self.Nvec * self.Nvec
                        offset = (
                            derivDict[self.derivTypeB[iterm]] * self.dataMomNum + momPosList[-k - 1]
                        ) * count * self.dtype_size
                        self.dataB[k, iterm] = cupy.fromfile(
                            self.elementalFile, dtype=self.dtype, offset=offset, count=count
                        ).reshape(self.TimeSlice, self.Nvec, self.Nvec)
...
class ForwardPeram(LatticeDesc):
    ...
    def load(self, iload):
        ...
        self.data = cupy.fromfile(
            self.perambulatorFile, dtype=self.dtype, count=count, offset=iload * count * self.dtype_size
        ).reshape(self.tSub, self.TimeSlice, self.spin, self.spin, self.Nvec, self.Nvec)
        time0.end()
        ...
        for iElemSrc in range(self.varDim):
            for iElemSink in range(self.varDim):
                ...
                if True:  #iElemSink >= iElemSrc:   ## 20220218: calc all off-diag!!!
                    savepath = self.corrSaveDir + "/" + self.operatorVarList[iElemSrc][
                        "name"] + "_" + self.operatorVarList[iElemSink]["name"]
